...
class PageController extends AbstractController
{
    ...
    public function block(Request $request, FileUploader $fileUploader, SessionInterface $session): Response
    {
        ...
        if ($request->get('title') && $request->get('subtitle')) {
            $data = json_decode($context_record->getJsonData());
            if (!isset($data->published_at)) {
                $request->request->set('published_at', date('Y-m-d'));
            } else {
                $request->request->set('published_at', $data->published_at);
            }
        }
