...
class IndexController extends AbstractController
{
    ...
    {
        $datas = $this->_page('accueil');

        $datas['slug'] = $this->slug;

        return $this->redirectToRoute('app_page', ['slug' => 'home']);
    }
     ...
     * @return Response
     */
    public function page($slug, $id = '', Request $request, MenuRepository $menuRepository, PageBlockRepository $pageBlockRepository, LangService $langService, SessionInterface $session)
    {
        $page = $this->getDoctrine()->getRepository(Page::class)->findBy(['slug' => $slug]);
        $pageDark = $this->getDoctrine()->getRepository(Page::class)->findBy(['slug' => 'dark']);

        $imgPreloads = $this->imgPreload();
        if (!empty($page)) {
          ...
          /*  if (2 === $lang) {
            ...
            $datas['blocks'] = array_merge($this->_getDataPage($slug, $request->getLocale(), $page[0]->getId()), $this->_getDataPage($slug, $request->getLocale(), $pageDark[0]->getId()));
     ...
     * @return array
     */
    protected function _page($page, $id = '')
    {
        return ['blocks' => hlp\Datas::getStructure($page, $id)];
    }
     ...
     */
    public function validOfferForm(Request $request, SaferpayService $saferpayService, Mail $mail)
    {
        $datas = $request->request->all();

        if (isset($datas['gift']) && $datas['gift'] && $datas['gift']['amount']) {
            $link = $saferpayService->getLink($datas['gift']['amount']);
...
//yaz adds
    ...
    protected function imgPreload(){
        $path =  __DIR__ .'/../../../public/build/images';
        $files = scandir($path);
        array_shift($files);
        array_shift($files);
        $array= array();
        foreach( $files as $file){
          if((int)substr($file,0,1) == 6 || (int)substr($file,0,1) == 7){
              $array[] = $file;
          }
       }
       return $array;
